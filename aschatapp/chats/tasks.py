...
@shared_task
def save_message(chat_id, user_id, content, timestamp, image_data=None, audio_data=None):
    try:
        ...
        if audio_data:
            logger.info(f"Processing audio data for message in chat {chat_id}")
            try:
                ...
                filename = f'message_audio_{chat_id}_{user_id}_{timestamp}.{ext}'
                audio_file = ContentFile(base64.b64decode(audiostr), name=filename)
                message.audio = audio_file
                logger.info(f"Audio will be saved as: {filename}")
            except Exception as e:
